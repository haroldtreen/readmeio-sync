...
var filterContent = function(customContent) {
    ...
    return {
        appearance: {
            html_head: appearance.html_head,
    ...
    var recursiveRequest = function(resources) {
        ...
        uploadResource(resource, function(err, response, body) {
            if (err || response.statusCode !== 200) {
                ...
                if (resource.appearance && resource.appearance.html_head) {
                    fs.writeFileSync(resource.appearance.html_head, respBody.appearance.html_head);
                }
                if (resource.appearance && resource.appearance.stylesheet) {
                    fs.writeFileSync(resource.appearance.stylesheet, respBody.appearance.stylesheet);
                }
            }
...
Requestor.prototype.uploadContent = function(resources, cb) {
    var urlFns = { post: 'contentPutUrl', put: 'contentPutUrl' };
    var createBodyFn = function(resource) {
        return JSON.stringify({
            appearance: {
                html_head: fs.readFileSync(resource.appearance.html_head).toString(),
