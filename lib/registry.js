...
Registry.prototype.markUploaded = function() {
    var markUploaded = function(content) { content.isUploaded = true; };

    this.allDocCategories().forEach(markUploaded);
    this.allDocs().forEach(markUploaded);
    this.allCustomPages().forEach(markUploaded);
};
